...
int main() {
    ...
    shared_ptr<force> grav = make_shared<force>();
    shared_ptr<force> grav1 = make_shared<force>();
    listForces force1 = {grav, grav1};
    vec3d v1(1., 2., 3.);
    vec3d v2(-1., -2., -3.);
    vec3d v3(4., 5., 6.);
    vec3d v4(0, 0, 0);
    particle p1(1., fluid1, force1, v1, v2, v3);
    prox.add(p1);
    cout << prox.get(v1) << " " << prox.get(v2) <<  " " << prox.get(v3) << endl;
    for (auto i: p1.forces)
        cout << i << ' ';
    cout << endl;
    for (auto j: prox.map) {
        cout << j.first << " ";
