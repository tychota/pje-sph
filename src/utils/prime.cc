 ...
 */
static int64_t pow_mod(int64_t a, int64_t x, int64_t n)
{
    /*
 ...
 *
 */
static int64_t rand_between(int64_t a, int64_t b)
{
    // Assume rand_func() is 32 bits
    int64_t r = (static_cast<int64_t>(rand())<<32) | rand();
    return a + (int64_t)((double)(b-a+1)*r/(INT64_MAX+1.0));
}
...
bool prime::isprime(int64_t n, int k)
{
    ...
    int s = 0;
    for ( int64_t m = n-1; !(m & 1); ++s, m >>= 1 )
        ; // loop

    int64_t d = (n-1) / (1<<s);

    for ( int i = 0; i < k; ++i ) {
        int64_t a = rand_between(2, n-2);
