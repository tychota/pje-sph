...
KernelBox::KernelBox(double smoothingLength) : Kernel(smoothingLength) {
    ...
    factorW = scaleFactorW * pow(reverseSmoothingLenght, 3);
...
double KernelBox::laplacian(double distance, const Vec3d& distanceVector) { throw NotImplementedException(); }

double KernelBox::maxDistance() { return smoothingLength; }